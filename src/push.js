...
export default class App extends Component {
  ...
  componentDidMount() {
    ...
    this.receivingDataMessage();
    console.warn(RNRemoteMessage().DATA);
  ...
  getToken() {
    ...
    NativeModules.RNHmsInstanceId.getToken((result, token) => {
      console.log('App -> getToken -> token', token);
      const spendTime = new Date().getTime() - startTime;
      let msg = '[spend ' + spendTime + 'ms]';
  ...
  render() {
    const {receiveContent} = this.state;
    console.log('App -> render -> receiveContent', receiveContent);
    return (
      <ScrollView>
        <Button
